...
for d = 1:nDeploy
    ...
    for k = 1:numel(l4Files)
        ...
        try
            ...
            % Guard (added 2026-07-26): every step below mixes the STORED
            % L4.eta with the CURRENT L1/L2. If L2's segment grid has moved
            % since this L4 was built, those are different hours and the
            % result is silently wrong. Three catalog records hit exactly this
            % after the 07-10/11 rerun. Refuse rather than mix; rebuild the
            % record from scratch instead (see PUV_L4_driver).
            [~, ainfo] = l4_l2_index_map(L2, L4);
            if ~ainfo.identity
                fprintf(['  [%d/%d] %s — SKIPPED: L4 grid does not match L2 ' ...
                         '(nL4=%d nL2=%d maxOffset=%d). Rebuild, do not refresh.\n'], ...
                    k, numel(l4Files), instr, ainfo.nL4, ainfo.nL2, ainfo.maxOffset);
                nFail = nFail + 1;
                continue
            end
