...
% Author: Holden Leslie-Bole, 2026
    ...
    % Reject on the BULK spread, not the single worst file. A dying battery
    % pulls the real-time clock off in the last hours of a deployment, so a
    % handful of end-of-record files can be far out while the whole rest of the
    % deployment is tight — TOR15B has two such files (~1180 s and ~582 s) out
    % of 924, with everything else within 18 s. Judging the reconstruction by
    % max(dev) would throw away 922 good files for 2 bad ones. The 98th
    % percentile ignores that thin tail while still catching a genuinely wrong
    % filename convention, where most files would be off.
    p98 = prctile(dev, 98);
    nOutlier = sum(dev > maxSpread);
    if p98 > maxSpread
        error('vec_clock_from_filenames:inconsistentOffset', ...
            ['Filename-to-RTC offset is not constant: median %.0f s, but the ' ...
             '98th-percentile deviation is %.0f s across %d files (limit %.0f s). ' ...
             'Most files disagree with the filenames, so the reconstruction ' ...
             'would be wrong — inspect the raw file names before processing.'], ...
            med, p98, numel(offsets), maxSpread);
    end
    if nOutlier > 0
        % Accepted, but say so: these files get timestamps up to max(dev) off.
        warning('vec_clock_from_filenames:tailOutliers', ...
            ['%d of %d files deviate more than %.0f s from the median offset ' ...
             '(worst %.0f s), consistent with clock degradation near battery ' ...
             'death. Their wave statistics are unaffected, but their absolute ' ...
             'timestamps are approximate.'], ...
            nOutlier, numel(offsets), maxSpread, max(dev));
    end
