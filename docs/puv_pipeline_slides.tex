...
\begin{frame}{L3 Architecture}
  \begin{itemize}
    \item \textbf{L3a}: Frequency-band energy decomposition (IG / swell / sea)
    \item \textbf{L3b}: Storm event detection with MOP gap-filling
    \item \textbf{L3c}: Transport proxies ($F_b$, Shields, Rouse, mobilization)
    \item \textbf{L3d}: Tidal current decomposition (t\_tide + NOAA depth)
    \item All components verified: $E_f\text{\_total} / \text{L2}.E_f = 1.0000$
          for all 33 instruments
  \end{itemize}
\end{frame}
...
\begin{frame}{Seasonal Energy Partitioning}
  \begin{itemize}
    \item \textbf{Winter}: swell-dominated (65--90\% of energy flux)
    \item \textbf{Summer}: sea-dominated or equal ($\sim$50/50)
...
\begin{frame}{Storm Detection + MOP Gap-Filling}
  \begin{itemize}
    \item NN24 Dec 28, 2023: $H_s$ up to 3.6~m, 74-hour duration
    \item MOP fills data gaps from instrument damage during storms
    \item Example: MOP651\_7m detected 0 PUV storms, MOP found 3
          (86-hour event)
    \item Critical for characterizing the forcing that damaged
          the instruments
  \end{itemize}
\end{frame}
...
\begin{frame}{Transport Proxy Depth Dependence}
  \begin{itemize}
    \item Shields parameter decreases offshore: 0.24 (5.6~m) to
          0.065 (15.5~m)
    \item Mobilization: 100\% at 5~m, 78\% at 15~m (winter)
    \item Rouse number: bedload-dominated across all sites
    \item Cumulative $F_b$: winter $3\times$ summer at equivalent depth
  \end{itemize}
\end{frame}
...
\begin{frame}{Tidal Validation}
  \begin{itemize}
    \item NOAA Scripps Pier tide gauge: $R = 0.995$ with PUV depth
    \item Confirms PUV clocks are UTC (lag $= 0.0$~hours)
    \item Subtidal cross-shore current: consistently offshore (undertow)
    \item Tidal currents small ($<$0.05~m/s) vs.\ wave-driven flows
  \end{itemize}
\end{frame}
...
\begin{frame}{TBR23 Context: Post-Storm Recovery}
  \begin{itemize}
    \item Winter 2023 storms deposited $\sim$85~cm sand at 5~m depth
    \item TBR23 deployment (May--Aug 2023) captures recovery phase
    \item 22 jetski surveys + 4 PUV instruments at 5~m and 7~m depth
    \item \textbf{Question}: what wave processes drive the recovery?
  \end{itemize}
\end{frame}
...
\begin{frame}{Competing Transport Mechanisms}
  \begin{itemize}
    \item \textbf{Onshore: velocity skewness} ($S_{k} > 0$) ---
          peaked crests drive bedload shoreward
    \item \textbf{Offshore: undertow} (mean return flow) ---
          opposes recovery
    \item Both present simultaneously at all instruments
    \item Balance determines net transport direction
  \end{itemize}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}{Transport Analysis}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{/Users/holden/Documents/Scripps/Research/Paper_1/paper/figures/fig_transport_mechanisms_v4.png}
  \end{center}
\end{frame}
...
\begin{frame}{Recovery Boundary at 6--7~m Depth}
  \begin{itemize}
    \item At 5.5~m: skewness flux 3--4$\times$ stronger than
          undertow $\rightarrow$ net onshore (recovery)
    \item At 8.3~m: undertow slightly dominates $\rightarrow$
          near-equilibrium
    \item Crossover at $\sim$6--7~m depth
    \item Explains: mid-July recovery pulse at 5~m site but not
          at 7~m sites
    \item Tidal modulation provides depth range at each fixed
          instrument
  \end{itemize}
\end{frame}
...
\begin{frame}{Key Findings}
  \begin{enumerate}
    ...
    \item \textbf{Full TBR23 analysis}: identified competing skewness
          vs.\ undertow transport, recovery boundary at 6--7~m depth
  \end{enumerate}
\end{frame}
...
\begin{frame}{Next Steps}
  \begin{itemize}
    \item \textbf{Bailard / Hoefel \& Elgar transport model}:
          implementation for quantitative flux estimates
    \item \textbf{L5 PUV--altimeter integration}: reconciled plan
          ready for deployment
    \item \textbf{IG / bound-wave analysis (L4)}: separate forced
          vs.\ free IG energy
    \item \textbf{TBR23 paper draft}: synthesize validation,
          transport, and morphology results
  \end{itemize}
\end{frame}
